// Region_seeds_growing.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//
...
void on_MouseHandle(int event, int x, int y, int flags, void* param) {
	...
	{
		cv::circle(src, cv::Point2i(x, y), 1, cv::Scalar(0, 0, 255), -1, LINE_AA);
		sprintf_s(str, "(%d,%d)", x, y);
		//cv::putText(src, str, cv::Point2i(x, y), 3, 1, cv::Scalar(150, 200,0), 2, 8);
		cv::namedWindow("dst", WINDOW_NORMAL);//定义一个dst窗口 
		pt = cv::Point2i(x, y);
...
int main() {
	cv::Mat src = cv::imread("E:\\PHD_FILE\\GitHub\\Texture\\texture_19_9_18\\images\\0.bmp");
	if (src.empty()) {
		return -1;
	}
	cv::namedWindow("src", WINDOW_NORMAL);//定义一个img窗口
